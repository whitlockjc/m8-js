...
describe('index tests', () => {
  describe('#dumpInstrument', () => {
    test('2.7.x', () => {
      Object.keys(m8Files['2.7.x'].instruments).forEach((fileName) => {
        const filePath = path.join(__dirname, `files/2.7.x/Instruments/${fileName}`)
        const bytesFromDisk = Array.from(readFileSync(filePath))
        const diskFileReader = new M8FileReader(bytesFromDisk)
        const instrFromDisk = M8.loadInstrument(diskFileReader)

        // Since this instrument was read from disk, we should be able to ensure the generated bytes are identical
        expect(bytesFromDisk).toEqual(M8.dumpInstrument(instrFromDisk, diskFileReader))

        let bytesFileReader = new M8FileReader(M8.dumpInstrument(instrFromDisk, diskFileReader))
        let alteredInstr = M8.loadInstrument(bytesFileReader)

        expect(alteredInstr).toEqual(m8Files['2.7.x'].instruments[fileName])
        ...
        bytesFileReader = new M8FileReader(M8.dumpInstrument(alteredInstr))
        alteredInstr = M8.loadInstrument(bytesFileReader)
  ...
  test('#dumpScale', () => {
    ...
    expect(bytesFromDisk).toEqual(M8.dumpScale(scaleFromDisk, diskFileReader.m8Version))

    let bytesFileReader = new M8FileReader(M8.dumpScale(scaleFromDisk))
  ...
  describe('#dumpSong', () => {
    test('2.7.x', () => {
      ...
      // Ensure the raw bytes read from disk match the dumped bytes
      expect(bytesFromDisk).toEqual(M8.dumpSong(songFromDisk, diskFileReader))

      let bytesFileReader = new M8FileReader(M8.dumpSong(songFromDisk))
  ...
  test('#dumpTheme', () => {
    ...
    // Ensure the raw bytes read from disk match the dumped bytes
    expect(bytesFromDisk).toEqual(M8.dumpTheme(themeFromDisk, diskFileReader.m8Version))

    let bytesFileReader = new M8FileReader(M8.dumpTheme(themeFromDisk))
  ...
  describe('#loadInstrument', () => {
    describe('2.7.x', () => {
      ...
      test('invalid instrument type', () => {
        ...
        fileReader.bytes[fileReader.cursor] = 0xFE
