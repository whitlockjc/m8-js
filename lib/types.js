...
// TODO: Add constants support (chain length, track length, ...)
// TODO: Add debug support
// TODO: Add error handling
// TODO: Change from using "rows" to type-specific storage of values (Chain, Phrase, ...) where possible
// TODO: Check for unused objects
// TODO: Create a Project type?
// TODO: Do we want to generate constants from arrays?
// TODO: Describe the class constructor parameters
// TODO: Refactor classes (We've cloned m8i.nim as-is)
// TODO: Refactor error values to use hex
// TODO: Remove all uses of Buffer
// TODO: Revisit const/class names
// TODO: Verify enum value types
...
class M8Version {
  ...
  toString () {
    // Convert the version number to padded hex
    const majorAsHex = this.majorVersion.toString(16).padStart(2, '0')
    const minorAsHex = this.minorVersion.toString(16).padStart(2, '0')
    const patchAsHex = this.patchVersion.toString(16).padStart(2, '0')

    return `${majorAsHex}.${minorAsHex}.${patchAsHex}`
  }
...
class FMSynthParameters {
  ...
  constructor (version, fileReader) {
    ...
    if (version.compare(VERSION_1_4_0) >= 0) {
  ...
  /**
   * Returns a string representation of the wave.
   *
   * @param {String} wave - The wave shape
   *
   * @returns {String}
   */
  getWaveStr (wave) {
...
class LFOParameters {
  ...
  constructor (fileReader) {
    ...
    // These are the LFO Shape names as of 2.6.0
    this.#lfoShapeNames = [
      'TRI', // 0x00
      'SIN', // 0x01
      'RAMP DN', // 0x02
   ...
   */
  getShapeStr () {
    return this.#lfoShapeNames[this.shape] || `UNKNOWN (${this.shape})`
  }

  getTriggerModeStr () {
    return this.#triggerModeNames[this.triggerMode] || `UNKNOWN (${this.this.triggerMode})`
  }
}
...
class MacrosynthParameters {
   ...
   */
  constructor (fileReader) {
    this.#shapeNames = MACROSYNTHShapes
    ...
    if (fileReader.m8Version.compare(VERSION_2_6_0) < 0) {
      this.#shapeNames = MACROSYNTHShapesPre260
    }
  }
   ...
   */
  getShapeStr () {
    const shapeName = this.#shapeNames[this.shape]
...
class Instrument {
  ...
  constructor (version, fileReader) {
    ...
    this.lfo = new Array(version.compare(VERSION_1_4_0) >= 0 ? 2 : 1)
    this.version = version
    this.#filterTypeNames = FilterTypes
    ...
    // TODO: Handle reading Instrument outside of a song
    ...
    switch (this.kind) {
      ...
      case 3:
        ...
        break
      case 4:
        this.instrParams = new FMSynthParameters(version, fileReader)
        break
      case 0xFF:
        // Do nothing
        break
    ...
    // Manually forward the file reader
    // TODO: Look into this
    ...
    if (this.kindToStr() === 'WAVSYNTH') {
      if (this.version.compare(VERSION_2_5_1) >= 0) {
        this.#filterTypeNames = FilterTypes.concat(WAVSYNTHFilterTypes)
      } else {
        this.#filterTypeNames = FilterTypesPre251.concat(WAVSYNTHFilterTypes)
      }
    }
  }
   ...
   */
  filterTypeToStr () {
    const filterName = this.#filterTypeNames[this.filterParams.type]
...
class Scale {
  ...
  offsetToStr (num) {
    ...
    if (typeof offset === 'undefined') {
      throw new RangeError(`There is no Scale offset number: ${num}`)
    }
...
class Song {
  ...
  constructor (fileReader) {
    ...
    // TODO: Should we rewrite how we handle empty objects?
    ...
    for (let i = 0; i < this.instruments.length; i++) {
      this.instruments[i] = new Instrument(this.version, fileReader)
