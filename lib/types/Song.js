...
class Song extends M8File {
  ...
  constructor (m8FileReaderOrVersion) {
    if (typeof m8FileReaderOrVersion === 'undefined') {
      super(M8FileTypes.Song, LATEST_M8_VERSION)
    } else {
      if (m8FileReaderOrVersion.constructor.name === 'M8FileReader') {
        super(m8FileReaderOrVersion)
      } else {
        super(M8FileTypes.Song, m8FileReaderOrVersion)
      }
    }
    ...
    for (let i = 0; i < this.instruments.length; i++) {
      this.instruments[i] = new None(this.m8FileReader)
    }
  ...
  asBytes () {
    ...
    for (let i = startLen + this.directory.length + 1; i < startLen + 128; i++) {
      ...
      if (this.m8FileReader?.skipped.indexOf(i) > -1) {
        skippedValue = this.m8FileReader.bytes[i]
      }
    ...
    bytes.push(this.key)
    bytes.push(...bytesForSkippedData(this.m8FileReader, bytes.length, 18, 0x00))
    ...
    // Skipped data
    bytes.push(...bytesForSkippedData(this.m8FileReader, bytes.length, 5, 0x00))
    ...
    // Tables
    for (let i = 0; i < this.tables.length; i++) {
      bytes.push(...this.tables[i].asBytes())
    }

    // Instruments
    for (let i = 0; i < this.instruments.length; i++) {
      bytes.push(...this.instruments[i].asEmbeddedBytes(bytes.length, false))
    }

    // Skipped data
    bytes.push(...bytesForSkippedData(this.m8FileReader, bytes.length, 3, 0x00))
    ...
    // Skipped data
    bytes.push(...bytesForSkippedData(this.m8FileReader, bytes.length, 1, 0x00))
    ...
    bytes.push(...bytesForSkippedData(this.m8FileReader, bytes.length, 0x1A5FE - bytes.length, 0xFF))
    ...
    if (this.m8FileVersion.compare(VERSION_2_5_0) >= 0) {
      ...
      bytes.push(...bytesForSkippedData(this.m8FileReader, bytes.length, 0x1AA7E - bytes.length, 0xFF))

      for (let i = 0; i < this.scales.length; i++) {
        bytes.push(...this.scales[i].asBytes(true))
      }
    } else {
      bytes.push(...bytesForSkippedData(this.m8FileReader, bytes.length, 256, 0x00))
    }
  ...
  static fromBytes (bytes) {
    ...
    for (let i = 0; i < song.midiMappings.length; i++) {
      ...
      midiMapping.instrIndex = m8FileReader.read()
