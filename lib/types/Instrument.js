...
class FMSynthParameters {
   ...
   *
   * @param {module:m8-js/lib/types.M8Version} [m8Version] - The M8 version can affect the bytes generated
   *
  ...
  getBytes (m8Version) {
    ...
    bytes.push(this.algo)

    // If supported, write the synth shapes
    if (m8Version.compare(VERSION_1_4_0) >= 0) {
      for (let i = 0; i < this.operators.length; i++) {
        bytes.push(this.operators[i].shape)
      }
    }
    ...
    bytes.push(this.mod1)
    bytes.push(this.mod2)
    bytes.push(this.mod3)
    bytes.push(this.mod4)
...
class MacrosynthParameters {
  ...
  getBytes () {
    ...
    bytes.push(this.shape)
    bytes.push(this.timbre)
    bytes.push(this.color)
    bytes.push(this.degrade)
    bytes.push(this.redux)
...
class MIDIOutParameters {
  ...
  getBytes () {
    ...
    bytes.push(this.port)
    bytes.push(this.channel)
    bytes.push(this.bankSelect)
    bytes.push(this.programChange)

    // Write 3 empty bytes (unused data)
    bytes.push(0x00, 0x00, 0x00)

    for (let i = 0; i < this.customCC.length; i++) {
      const customCC = this.customCC[i]
...
class SamplerParameters {
  ...
  getBytes () {
    ...
    bytes.push(this.playMode)
    bytes.push(this.slice)
    bytes.push(this.start)
    bytes.push(this.loopStart)
    bytes.push(this.length)
    bytes.push(this.degrade)

    // The sample path IS NOT represented here and is serialized to bytes directly in Instrument#getBytes().
...
class WavsynthParameters {
  ...
  getBytes () {
    ...
    bytes.push(this.shape)
    bytes.push(this.size)
    bytes.push(this.mult)
    bytes.push(this.warp)
    bytes.push(this.mirror)
...
class Instrument extends M8File {
   ...
   */
  constructor (m8ReaderOrVersion, kind, kindStr, instrParams) {
    ...
    super(M8FileTypes.Instrument, m8Version)
  ...
  getBytes () {
    ...
    bytes.push(...this.getEmbeddedBytes(bytes.length))
   ...
   * @param {module:m8-js/lib/types.M8FileReader} [fileReader] - The M8 file reader (when present)
   *
   * @returns {Array<Number>}
   */
  getEmbeddedBytes (offset, fileReader) {
    const bytes = []
    ...
    if (typeof this.instrParams !== 'undefined') {
      bytes.push(...this.instrParams.getBytes(this.m8FileVersion))
    }
    ...
    bytes.push(...bytesForSkippedData(fileReader, bytes.length + offset, (offset + 0x57) - (offset + bytes.length), 0xFF))
    ...
    if (typeof fileReader === 'undefined' || fileReader.fileTypeToStr() === 'Instrument') {
      bytes.push(...this.tableData.getBytes())
    }
