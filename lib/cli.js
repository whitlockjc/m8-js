...
const { getNote, toM8HexStr, toM8Bool, toM8Num } = require('../lib/helpers')
const { loadM8File } = require('..')
const { VERSION_1_4_0 } = require('../lib/constants')
...
function getM8FileReaderAndVerify (filePath, type) {
  ...
  if (typeof type === 'string') {
    if (fileReader.fileType !== type) {
      throw new commander.InvalidArgumentError(`m8-file must be a ${type} file`)
    }
  } else {
    if (typeof type !== 'undefined' && type.indexOf(fileReader.fileType) === -1) {
...
`
        ...
        if (instrument.version.compare(VERSION_1_4_0) >= 0) {
          instrumentData += '        '
